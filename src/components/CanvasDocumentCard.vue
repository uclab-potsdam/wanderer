...
const x = computed(() => dragX.value + props.node.x);
const y = computed(() => dragY.value + props.node.y);

const connectable = computed(() => props.node.item["@type"] !== "Canvas");
...
function updateArrowDeg(e) {
  const vectorX =
    e.x / props.transform.k -
    (props.node.x + props.transform.x / props.transform.k);
  const vectorY =
    e.y / props.transform.k -
    (props.node.y + props.transform.y / props.transform.k);
  arrowDeg.value =
    Math.atan(vectorY / vectorX) * (180 / Math.PI) + (vectorX > 0 ? 0 : 180);
}

async function deleteNode() {
  terminusStore.deleteDocument(props.node["@id"]);
  const index = terminusStore.nodes.findIndex(
    (n) => n["@id"] === props.node["@id"]
  );
  if (index >= 0) {
    terminusStore.nodes.splice(index, 1);
  }
}
  ...
  <foreignObject width="100" height="100" :x="x" :y="y">
    ...
    >
      ...
      <template v-slot:right>
        ...
        <RouterLink :to="`/edit/${node.item['@id']}`">
          <IconEdit />
        </RouterLink>
      </template>
    </DocumentCard>
